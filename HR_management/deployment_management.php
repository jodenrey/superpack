...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['update_deployment'])) {
        ...
        $stmt->bind_param("ssdddsi", $department, $position, $salary, $otRate, $leaveEntitlement, $status, $deploymentId);
        ...
        if ($stmt->execute()) {
            ...
            // Update the employee_records table
            $updateEmployeeQuery = "UPDATE employee_records SET 
                                  position = ?, 
                                  department = ?,
                                  status = ?
                                  WHERE id = ?";
            ...
            $employeeStatus = ($status === 'Active') ? 'Active' : 'Inactive';
            $employeeStmt->bind_param("sssi", $position, $department, $employeeStatus, $employeeId);
    ...
    if (isset($_POST['add_deployment'])) {
        ...
        if ($checkResult->num_rows > 0) {
            ...
            if ($stmt->execute()) {
                ...
                $employeeStmt = $conn->prepare($updateEmployeeQuery);
                $employeeStmt->bind_param("ssi", $position, $department, $employeeId);
                $employeeStmt->execute();
...
// Base query to retrieve deployments
