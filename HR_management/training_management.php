    ...
    if (isset($_POST['update_training'])) {
        ...
        if ($stmt->execute()) {
            ...
            if ($trainingStatus === 'Completed' && $completionDate) {
                ...
                $insertEmployeeQuery = "INSERT INTO employee_records (name, position, email, phone, address, gender, education, department, hire_date) 
                                      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
                ...
                if ($stmt->execute()) {
                    ...
                    $deploymentStmt->bind_param("isss", $employeeId, $department, $candidate['position'], $hireDate);
