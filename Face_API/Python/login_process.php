...
try {
    ...
    if ($login_method === 'qr') {
        ...
        $stmt = $conn->prepare("SELECT id, first_name, middle_name, last_name, role, department, qr_code_data FROM register WHERE employee_id = ?");
        ...
        if ($result->num_rows === 1) {
            ...
            if ($is_valid) {
                ...
                $_SESSION['name'] = $full_name; // Use reconstructed full name
        ...
        // Check credentials
        $stmt = $conn->prepare("SELECT id, first_name, middle_name, last_name, role, department, password FROM register WHERE employee_id = ?");
        $stmt->bind_param("s", $employee_id);
        $stmt->execute();
        $result = $stmt->get_result();
...
function decryptQRData($qr_data) {
    ...
    // When decrypting, the 'n' field from QR still represents the full name as it was generated by register_process.php
    $full_data = [
        'employee_id' => $data['id'] ?? '',
        'name' => $data['n'] ?? '', // This 'name' is the full name from QR
        'role' => $data['r'] ?? '',
        'department' => $data['d'] ?? '',
        'timestamp' => $data['t'] ?? 0
    ];
